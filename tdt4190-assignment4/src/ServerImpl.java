...
public class ServerImpl extends UnicastRemoteObject implements Server
{
  ...
  synchronized Server getServer(int id)
  {
    ...
    public void receiveProbe(Queue<Integer> probe) throws RemoteException {
        if(this.activeTransaction != null){
            ResourceAccess waitingFor = this.activeTransaction.getWaitingForResource();
            if(waitingFor != null){
                if(probe.contains(this.activeTransaction.getId())){
                    //Possible deadlock
                    probe.add(this.activeTransaction.getId());
                    println("Dead lock detected" + probe, this.activeTransaction.getId());
                    println("Size of waitForGraph: " + probe.size());
                    endCurrentTransaction();
                }else {
                    println("Waiting for other transactions, sending probe messages to them");
                    probe.add(this.activeTransaction.getId());
                    //send message to the server we are waiting for
                    waitingFor.server.receiveProbe(probe);
                }
            }
    ...
    public void notifyResource(int resourceID) throws RemoteException {
        Resource resource = (Resource) this.resources.get(resourceID);
        //Prevent concurrency issues
        synchronized(resource){
            //Notify that resource is available
            resource.notify();
        }
    ...
    private void endCurrentTransaction(){
        println("Trying to resolve deadlock " + this.activeTransaction.getId());
        try{
            this.activeTransaction.getWaitingForResource().server.notifyResource(this.activeTransaction.getWaitingForResource().resourceId);
  ...
  {
    String registryAddress = "localhost:1340";
